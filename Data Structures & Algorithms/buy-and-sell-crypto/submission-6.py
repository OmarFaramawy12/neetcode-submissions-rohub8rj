class Solution:
    def maxProfit(self, prices: List[int]) -> int:
        ...
        while right < len(prices):
            ...
            # case where we can't make the transaction (we want the left pointer (buying time) be the minimum)
            # case where selling is lower than buying (negative profit) --> we update buying time to be point at lowest 
            # price we have found until now
            else:
                left  = right
